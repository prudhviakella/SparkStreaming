package spark.streaming.structured.d_Aggregations

import org.apache.spark.sql.SparkSession
import org.apache.spark.sql.types.{StringType, StructField, StructType}
import org.apache.spark.sql.functions._

object a_Aggregations extends App {
  ...
  * from the city of london.So lets create one droplocation directory to which our spark streaming will be listening to.*/
  ...
  //calculate the number of convictions that took place on per borough basis
  import spark.implicits._
  val convictionsPerBorough = Filedataset
    .groupBy("borough")
  ...
  //Entire result table is going to update for each trigger because outputMode is complete
